...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><a href="{{ route('taiKhoan.index') }}"><span class="text-muted fw-light">Danh sách /</span></a> Sửa tài khoản</h4>
        {!! @csrf_field() !!}
                ...
                <div class="card-body">
                    <form action="{{ route('taiKhoan.update', ['taiKhoan' => $taiKhoan]) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="HoTen" class="form-control" value="{{ $taiKhoan->ho_ten }}" id="exampleFormControlInput1" placeholder="Họ tên" />
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlSelect1" class="form-label">Số điện thoại</label>
                            <input type="text" name="SDT" class="form-control" value="{{ $taiKhoan->sdt }}" id="exampleFormControlInput1" placeholder="Số điện thoại" />
                        </div>
                        <div class="mb-3">
                            <label for="exampleDataList" class="form-label">Ngày sinh</label>
                            <input type="date" name="NgaySinh" class="form-control" min="1" value="{{ $taiKhoan->ngay_sinh }}" id="exampleFormControlInput1" placeholder="Ngày sinh" />
                        </div>
                        <div class="mb-3">
                            <label for="exampleDataList" class="form-label">Địa chỉ</label>
                            <input type="text" name="DiaChi" class="form-control" min="1" value="{{ $taiKhoan->dia_chi }}" id="exampleFormControlInput1" placeholder="Địa chỉ" />
                        </div>
                        <div class="mb-3">
                            <input type="text" hidden name="PhanLoaiTaiKhoan" class="form-control" placeholder="" value="{{ $taiKhoan->phan_loai_tai_khoan }}">
                        </div>
